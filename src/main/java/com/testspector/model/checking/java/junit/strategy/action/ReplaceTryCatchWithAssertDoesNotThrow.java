...
public class ReplaceTryCatchWithAssertDoesNotThrow implements Action<BestPracticeViolation> {
    ...
    public void execute(BestPracticeViolation bestPracticeViolation) {
        ...
        if (containsAssertionsClassImportStatement(javaFile)) {
            ...
            if (!containsAssertDoesNotThrowStaticImportStatement(javaFile)) {
                javaFile.getImportList().add(PsiElementFactory.getInstance(psiTryStatement.getProject())
                        .createImportStaticStatement(getAssertionsPsiClass(), "assertThrows"));
            }
        }
    ...
    private boolean containsAssertDoesNotThrowStaticImportStatement(PsiJavaFile javaFile) {
        return Optional.ofNullable(javaFile.getImportList())
                .map(importList -> Arrays.stream(importList.getImportStaticStatements())
                        .anyMatch(psiImportStatement -> "import static org.junit.jupiter.api.Assertions.assertDoesNotThrow;".equals(psiImportStatement.getText())))
                .orElse(false);
    }
